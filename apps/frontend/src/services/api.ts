...
export const api = {
  ...
  superAdmin: {
      ...
      getPackages: () => getCollection<SubscriptionPackage>('super-admin/packages'),
      savePackage: (pkg: any) => createDocument('super-admin/packages', pkg),
      deletePackage: (id: string) => deleteDocument('super-admin/packages', id),
      getTenants: () => getCollection<Tenant>('super-admin/tenants'),
      ...
      updateSuperAdmin: async (id: string, data: any) => {
          const response = await fetch(`${API_URL}/super-admin/users/${id}`, {
              method: 'PUT',
              headers: getHeaders(),
              body: JSON.stringify(data)
          });
      ...
      // ROLES
      getRoles: () => getCollection<any>('super-admin/roles'),
      createRole: (data: any) => createDocument('super-admin/roles', data),
      updateRole: (data: any) => updateDocument('super-admin/roles', data),
      deleteRole: (id: string) => deleteDocument('super-admin/roles', id)
  },
